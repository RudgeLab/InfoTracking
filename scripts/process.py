...
vel = np.load('velocity_data/vel.np.npy')
pos = np.load('velocity_data/pos.np.npy')
...
mask_all = imread('contour_masks/120_frames/10x_1.5x_-5_pAAA_MG_1_MMStack_Pos8.contour.mask.tif')
mask_all = mask_all>0
...
for frame in range(nt):
    ...
    for ix in range(nx):
        for iy in range(ny):
            ...
            radpos[frame,ix,iy] = np.nanmean(edt[frame, px[ix,iy]:px[ix,iy]+64, py[ix,iy]:py[ix,iy]+64])
...
# Save results
np.save('results/radpos.npy', radpos)
np.save('results/edt.npy', edt)
np.save('results/vmag.npy', vmag)
np.save('results/vrad.npy', vrad)
np.save('results/vtheta.npy', vtheta)
