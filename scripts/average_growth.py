...
#mask_all = imread('../../10x_1.5x_-5_pAAA_MG_1_MMStack_Pos9.ome.contour.mask.tif')
...
for t in range(nt):
    m = mask_all[t*step,:,:]
    edt[t,:,:] = distance_transform_edt(m)
    rmax[t] = edt[t,:,:].max()
...
vfront = savgol_filter(rmax, 15, 3, deriv=1)
np.save('vfront.npy', vfront)
exprate = savgol_filter(area, 15, 3, deriv=1) / savgol_filter(area, 15, 3)
